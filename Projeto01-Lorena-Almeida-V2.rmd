---
title: "Projeto 01 - Detecção de Fraudes no Tráfego de Cliques em Propagandas de Aplicações Mobile"
author: "Lorena França Almeida"
date: "22 de Agosto, 2021"
output: 
...
```{r Definição de Diretório e Importação da Amostra}
...
```{r Data Munging e Análise Exploratória}
# Explorando os dados
summary(amostra1)
...
# Confirmando divisão das quantidades de IPs que clicaram somente uma vez
...
por IPs que clicaram somente uma vez e 33% realizados por IPs que clicaram mais
...
uma coluna ao dataset da amostra classificando o IP como repetido = TRUE ou não repetido (único) = FALSE.
```{r}
...
```{r Criação e Score do Modelo Supervisionado de Machine Learning}
